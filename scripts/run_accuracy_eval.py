...
def main():
    ...
    total_iterations = len(GOLDEN_TEST_SET) * args.runs
    done = 0
    t_start = time.time()

    for run_idx in range(1, args.runs + 1):
        print(f"\n{'=' * 60}\n🔁 التشغيلة {run_idx}/{args.runs}\n{'=' * 60}")
        for job in GOLDEN_TEST_SET:
            done += 1
            print(f"  [{done}/{total_iterations}] {job['title']}...", end=" ", flush=True)
            try:
                triad = _run_triad(job, embedder, scorer, judge)
    ...
    flaky = [j["title"] for j in GOLDEN_TEST_SET
